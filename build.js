...
// the renewed line. Newlines survive (collapse like the renewed text does).
...
function renderFolio(r) {
  ...
  // Set in light by default; tap the verse to unfold the original beneath it —
  // muted, with only the renewed words struck. Native <details>: no JS, instant,
  // accessible (keyboard + screen reader), works with JS off. The renewed line
  // stays put; the witness appears below it.
  return `        <li class="cfolio">
...
${sections}
    ...
    <footer class="council-bar" aria-label="The Council">
      <a class="council-link" href="https://council.magisterium.church/form/dNuF1Bd1HTduxRJl4HRdQjVvl8gmDaK0C1OM84nd9V0" target="_blank" rel="noopener noreferrer">Submit a passage for renewal</a>
      <span class="council-sep" aria-hidden="true"></span>
      <a class="council-link" href="https://council.magisterium.church/form/aSdjJUVySYiBqgA5VmMZrfIfkhSQOxzST6CjKunqYSI" target="_blank" rel="noopener noreferrer">Volunteer for the Council</a>
    </footer>
  </div>

  <dialog id="council-modal" class="council-modal" aria-label="The Council">
    <div class="cm-panel">
      <button class="cm-close" type="button" aria-label="Close the form">
        <svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true"><path d="M6 6l12 12M18 6L6 18" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/></svg>
      </button>
      <iframe class="cm-iframe" title="Council submission form" src="about:blank"></iframe>
    </div>
  </dialog>
